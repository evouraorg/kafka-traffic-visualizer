...
const sketch = (p) => {
  ...
  function setupControlReferences() {
    ...
    processingSpeedSlider = p.select('#processingSpeedSlider');
    ...
    processingSpeedInput = p.select('#processingSpeedInput');
  ...
  function calculateRecordRadius(size) {
    ...
    return p.map(
        p.log(size),
        p.log(recordValueSizeMin),
        p.log(recordValueSizeMax),
      ...
      for (let i = 0; i < partition.records.length; i++) {
        ...
        if (i === 0) {
          ...
          if (record.x < maxPossibleX) {
            ...
            record.x = p.min(record.x + record.speed, maxPossibleX);
          }
        }
      }
